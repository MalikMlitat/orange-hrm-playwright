...
@pytest.mark.parametrize(
    "add_employee",
    [Employee(first="Malik"), Employee(last="Alipapa"), Employee()],
    indirect=True,
)
def test_api_employee_personal_details_visible(
    page: Page, login_with_admin, add_employee
):
    page.goto(
        f"/web/index.php/pim/viewPersonalDetails/empNumber/{add_employee.emp_number}"
    )
    expect(page.get_by_role("heading", name="Personal Details")).to_be_visible()


@pytest.mark.parametrize("add_employee", [Employee(first="Malik")], indirect=True)
def test_employee_created_via_api_is_visible_in_list(page: Page, add_employee):
    ...
    expect(page.get_by_role("heading", name="Personal Details")).to_be_visible()
    expect(
        page.get_by_text(f"{add_employee.first} {add_employee.last}")
    ).to_be_visible()
